...
const getAutourPageData = cache(async () => {
  ...
  const seasonalData = {
    ...
    activities: {
      winter: activitiesData?.filter((a: { season: string; }) => a.season === "winter") || [],
      summer: activitiesData?.filter((a: { season: string; }) => a.season === "summer") || [],
    },
    nearby: {
      winter: nearbyData?.filter((n: { season: string; }) => n.season === "winter") || [],
      summer: nearbyData?.filter((n: { season: string; }) => n.season === "summer") || [],
    },
  };
...
export default async function AutourPage() {
  ...
  return (
    <>
      <PageHero dataContent={content.PageHero} />
      <SeasonalWrapper data={seasonalData} />
      <LocalFavorites
